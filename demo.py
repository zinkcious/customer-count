...
def main1():
    cap = cv2.VideoCapture('1031_2.mov')
    minsize = 20
    ...
    while(1):
        ...
        boundingboxes, facial_points = detect_face(img_matlab, minsize, PNet, RNet, ONet, threshold, False, factor) 
        ...
        for point in points:
            ...
            for prevPoint in prev:
                ...
                if dist[0] <= 120:
                    if line2(xnew, ynew) > 0 and line2(xold, yold) < 0: # Point is passing the line
                        print('One Crossed')
                        print(point)
                        crossed += 1
                        crossPairs.add(((xnew,ynew), (xold,yold)))
        
        for point in points:
            (xnew, ynew) = point
            if line2(xnew, ynew) > 0:
                cv2.circle(img, point, 3, (255,0,255),6)
            else:
                cv2.circle(img, point, 3, (0,0,255),6)
        cv2.line(img, (0,450), (960,450), (255, 0, 0), 4)
        for pair in crossPairs:
            cv2.line(img, pair[0], pair[1], (255, 255, 255), 10)
        cv2.putText(img,'People Passed = '+str(crossed),(100,50), font, 1,(255,255,255),2,cv2.LINE_AA)
        cv2.imshow('img', img)
